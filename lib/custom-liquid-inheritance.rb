...
# author: Dylan M. maverickpuss At gmail Dot com
# required Liquid version: >= 3.0.6 (or earlier, haven't tested yet)
module CustomLiquidInheritance
  ...
  def self.load_template(tpl_name)
      source = Liquid::Template.file_system.read_template_file(tpl_name, nil)
      Liquid::Template.parse(source)
  end

  def self.find_blocks(parent, blocks=[])
    parent.nodelist.each do |node|
      if node.is_a?(BlockTag)
        ...
        if (blocks.index { |x| x.name == node.name })
          raise Liquid::SyntaxError.new("Syntax Error in 'block #{node.name}' - block can't be defined twice in the same layout")
        end
  ...
  def self.replace_nodes(parent, blocks)
